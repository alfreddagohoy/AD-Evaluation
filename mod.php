...
if(($act == 'ban') && $user->group->canban && ($user->data->userid != $u->userid)) {
	...
	if($_POST) {
		...
		if(($period > 0 && $period <= $set->max_ban_period) && isset($reason[5])) {
			...
			$db->query("UPDATE `".APP_PREFIX."users` SET `banned` = '1' WHERE `userid` = '$u->userid'");
			$db->query("INSERT INTO `".APP_PREFIX."banned` SET `userid` = ?i, `by` = ?i, `until` = ?i, `reason` = ?s", $u->userid, $user->data->userid, time()+$period, $reason);
			$page->success = "User has been successfully set to inactive for ".(int)$_POST['period']." day(s) ! ";
		} else {
		...
		<div class='controls'>
		<button id='submit' name='submit' class='btn btn-primary'>Set</button>
		</div>
		</div>
		...
		// if he is already active we show the inactive option
		if($u->banned) {
			...
			".$options->info("This user was set to <i>inactive</i> by <a href='$set->url/profile.php?u=$banned->by'>".$user->showName($banned->by)."</a> for `<i>".$options->html($banned->reason)."</i>`.",1)."
			<!-- Button -->
			<div class='control-group'>
			<label class='control-label' for='submit'></label>
			<div class='controls'>
			<button id='submit' name='submit' class='btn btn-primary'>Set Active</button>
			</div>
			</div>
	...
	$db->query("UPDATE `".APP_PREFIX."users` SET `banned` = '0' WHERE `userid` = ?i", $u->userid);
	$db->query("DELETE FROM `".APP_PREFIX."banned` WHERE `userid` = ?i", $u->userid);
	header("Location: ". $set->url."/profile.php?u=$u->userid");
	exit;
} else if(($act == 'avt') && $user->group->canhideavt) {
	if($u->showavt == 0){
		if($db->query("UPDATE `".APP_PREFIX."users` SET `showavt` = '1' WHERE `userid` = ?i", $u->userid))
		$_SESSION['success'] = 'Avatar showed successfully !';
	} else
	if($db->query("UPDATE `".APP_PREFIX."users` SET `showavt` = '0' WHERE `userid` = ?i", $u->userid))
	...
	if($_POST) { // we make sure that the users is deleted from all tables
		$db->query("DELETE FROM `".APP_PREFIX."users` WHERE `userid` = ?i", $u->userid);
		$db->query("DELETE FROM `".APP_PREFIX."privacy` WHERE `userid` = ?i", $u->userid);
