...
$page->title = "Students - ". $set->site_name;
$presets->setActive("studentslist"); // we highlight the home link
...
if($total_results = $db->getRow("SELECT COUNT(*) as count FROM `".APP_PREFIX."users` ?p", $where)->count) {
  ...
  $data = $db->getAll("SELECT * FROM `".APP_PREFIX."users` ?p ORDER BY ?p LIMIT ?i, ?i", $where, $order_by, $start, $perpage);
  $pagination = new pagination($total_results, $page_number, $perpage);

  foreach($data as $u) {
    $content .= "<li class='span5 clearfix'>
    <div class='thumbnail clearfix'>
    <a href='$set->url/profile.php?u=$u->userid'><img src='img/default-avatar.png' width='100' alt='".$options->html($u->username)."' class='pull-left clearfix' style='margin-right:10px'>
    <div class='caption' class='pull-left'>
    <h4>
    <a href='$set->url/profile.php?u=$u->userid'>".$user->showName($u->userid)."</a>
    </h4>
    <small><b>User Role: </b> ".$options->html($u->groupid)."</small><br/>
...
echo "
<div class='container'>
<legend><h2 class='pull-left'>Students List <a class='btn btn-success' href='$set->url'>Manage</a></h2></legend>

<form class='form-search' action='?'>
<div class='input-append pull-right'>
<input class='span2 search-query' name='q' type='text' ".( isset($_GET['q']) ? "value='".$options->html($_GET['q'])."'" : "" )." placeholder='Search Student'/>
